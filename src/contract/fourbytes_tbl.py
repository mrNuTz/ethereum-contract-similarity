import re
import contract.structure as structure
...
def signatureTbl(code):
    ...
    for reStart,reSignature,id in [(ST7,SI7,7), (ST1,SI1,1), (ST2,SI2,2), (ST3,SI3,3), (ST4,SI4,4), (ST5,SI5,5), (ST6,SI6,6), (ST8,SI8,8)]:
        ...
        while result:
            ...
            if result:
                groups = chunkList([ x for x in result.groups() if x is not None ], 2)
                for sig, dest in groups:
                    sig = sig.rjust(4,b'\x00')
                    dest = int.from_bytes(dest, 'big')
                    if sig in sigToAddr:
                        continue
                    sigToAddr[sig] = dest
        if sigToAddr:
